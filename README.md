...
- `model_prediction_example.py` - 模型預測使用範例與說明
...
訓練完成的模型會儲存在 `output_models/` 資料夾中，可以用於預測新的產品推薦。

### 📁 模型檔案說明

- `model_final.bin` - 完整的訓練模型（包含資料預處理 pipeline）
- 模型包含完整的特徵欄位資訊和預處理步驟
...
# Windows
.\.venv\Scripts\Activate.ps1
python model_prediction_example.py
```

```bash
# macOS/Linux
source .venv/bin/activate
python3 model_prediction_example.py
```
...
載入訓練好的模型檔案，獲取 pipeline 物件進行預測。

#### 2. 單一產品預測

準備包含所有必要特徵欄位的產品資料，轉換為 DataFrame 格式後進行預測。

**重要提醒**：產品資料必須包含訓練時使用的所有特徵欄位，欄位名稱需完全一致。

**檢查所需特徵欄位**：可載入模型查看 `feature_columns` 屬性來確認所需欄位。

#### 3. 批量預測

讀取包含多筆產品資料的 CSV 檔案，進行批量預測並輸出結果。
...
1. **資料格式**: CSV 檔案或 pandas DataFrame
2. **特徵欄位**: 必須包含模型訓練時使用的所有特徵欄位
3. **欄位名稱**: 必須與訓練時完全一致（區分大小寫）
4. **資料類型**:
   - 數值型欄位：整數或浮點數
   - 類別型欄位：字串
   - 布林型欄位：0/1 或 True/False
...
2. **probability_not_recommended** (float):

   - 範圍：0.0 ~ 1.0
   - 不推薦的機率

3. **probability_recommended** (float):

   - 範圍：0.0 ~ 1.0
   - 推薦的機率
   - 注意：兩個機率相加等於 1.0

4. **recommendation** (str):
   - "不推薦" 或 "推薦"
   - 基於 prediction 的文字描述
...
1. **欄位名稱不匹配**: 確保欄位名稱完全一致
2. **缺少必要欄位**: 所有訓練特徵都必須存在
3. **資料類型錯誤**: 數值欄位不能是字串
4. **編碼問題**: CSV 檔案使用 UTF-8 編碼

### 📊 模型輸出說明

- **prediction**: 0 (不推薦) 或 1 (推薦)
- **probability_recommended**: 推薦的機率 (0.0 ~ 1.0)
- **recommendation**: 文字描述 ("推薦" 或 "不推薦")

**詳細說明請參考上方的「模型輸入輸出規格」章節。**

### ⚠️ 重要注意事項

1. **特徵欄位一致性**: 預測資料必須包含訓練時使用的**所有**特徵欄位
2. **欄位名稱精確匹配**: 欄位名稱必須與訓練資料完全一致（區分大小寫）
3. **資料類型一致**: 確保數值型欄位為數字，類別型欄位為字串
4. **缺失值處理**: 模型可處理部分缺失值，但建議提供完整資料
5. **新資料適用性**: 模型適用於與訓練資料相似領域的新資料

#### 💡 **最佳實務建議**

1. **預測前驗證**: 確認輸入資料包含所有必要欄位
2. **小批量測試**: 先用少量資料測試，確認格式正確
3. **結果驗證**: 檢查預測機率範圍是否合理（0-1 之間）
4. **效能監控**: 大批量預測時注意記憶體使用量
5. **定期更新**: 當資料分布變化時，考慮重新訓練模型

## 重要提醒

### 環境設定

1. **Python 版本**: 需要 Python 3.10 或以上版本（此環境使用 Python 3.13.3）
...
├── traning_app.py              # 主程式
├── model_prediction_example.py # 模型預測使用範例
├── app_utils/                  # GUI 相關模組
│   ├── __init__.py
